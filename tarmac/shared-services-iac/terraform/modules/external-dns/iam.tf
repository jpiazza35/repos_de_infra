...
resource "aws_iam_policy" "external_dns" {
  name   = format("%s-external-dns-policy", lower(local.cluster_name[0]))
  path   = "/"
...
resource "null_resource" "add_worker_node_assume_role_policy" {
  ...
  provisioner "local-exec" {
    command = "sleep 5;aws iam update-assume-role-policy --role-name dns-manager --policy-document '${self.triggers.updated_policy_json}' --profile ss_network"
  }
