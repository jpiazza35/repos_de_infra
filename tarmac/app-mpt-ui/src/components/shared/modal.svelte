...
<script>
  ...
  const open = (NewComponent, options = {}, callback = {}) => {
  ...
  const handleKeydown = event => {
    if (state.closeOnEsc && Component && event.key === "Escape") {
      if (!modalValue?.hasUnSavedChanges) {
        ...
        jQuery("#dialog").kendoDialog({
          ...
          visible: false,
          width: 500,
          content: modalValue?.message || "You have unsaved changes which might be lost. Do you wish to continue?",
          actions: [
            {
              text: "Yes",
              primary: true,
      ...
      >
        {#if state.closeButton}
          {#if isFunction(state.closeButton)}
            <svelte:component this={state.closeButton} onClose={confirmClose} />
          {:else}
            <button
              id="popup-close"
        ...
        <div class={state.classContent} class:content={!unstyled} style={cssContent}>
          <svelte:component this={Component} on:close={confirmClose} on:dataUpdate={onDataUpdate} />
        </div>
      </div>
    </div>
  </div>
