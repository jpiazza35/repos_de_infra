...
resource "aws_cloudfront_distribution" "s3_bucket_distribution" {
  ...
  aliases = var.distribution_aliases_enable ? concat(var.distribution_aliases, var.distribution_external_aliases) : []
  ...
  ordered_cache_behavior {
    path_pattern     = var.distribution_ordered_path
    allowed_methods  = var.distribution_ordered_allowed_methods
    cached_methods   = var.distribution_ordered_cached_methods
    target_origin_id = local.s3_origin_id

    forwarded_values {
      query_string = var.distribution_ordered_forwarded_query_string
      headers      = var.distribution_ordered_forwarded_headers

      cookies {
        forward = var.distribution_ordered_forwarded_cookies
      }
    }

    min_ttl                = var.distribution_ordered_min_ttl
    default_ttl            = var.distribution_ordered_ttl
    max_ttl                = var.distribution_ordered_max_ttl
    compress               = var.distribution_ordered_compress
    viewer_protocol_policy = var.distribution_ordered_viewer_protocol
  }
  ...
  viewer_certificate {
    cloudfront_default_certificate = var.distribuion_viewer_certificate
  }
}