...
def lambda_handler(event, context):
    ...
    log_event = json.loads(cw_logs)
    message = log_event['logEvents'][0]['message']
    json_object = json.loads(message)
    username = json_object['userIdentity']['userName']
    print('Username on the failed login event is: {}'.format(username))
    ...
    if failed_logins < user_failed_logins_count['Count']:
        ...
        if date_first_bad_login_FMTD > past_10minutes :
            message="Detected failed login on the SSO service of the organization. username:{} timestamp: {}".format(username,cur_date_time)
            print(message)
