resource "aws_s3_bucket" "thanos" {
  bucket = "${lower(var.cluster_name)}-${var.prometheus_release_name}-thanos-bucket"

  tags = {
    Name        = "${lower(var.cluster_name)}-${var.prometheus_release_name}-thanos-bucket"
    Environment = var.environment
  }
}

resource "aws_iam_policy" "thanos" {
  name   = "${var.cluster_name}thanos_s3_policy"
  path   = "/"
