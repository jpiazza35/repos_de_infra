...
resource "vault_generic_secret" "token_secret_devops_vault" {
  data_json = jsonencode({
    id                               = databricks_obo_token.token.id
    token                            = databricks_obo_token.token.token_value
    service_principal_application_id = var.service_principal_application_id
  ...
  path = "${local.devops_vault_base_path[var.env]}/databricks/service-principal-tokens/${var.service_principal_name}"
}
