<script>
  ...
  $: disabledFilter = invalidSpecialty
  ...
  <div class="h-full overflow-auto">
    <div class="flex flex-wrap items-center px-4 py-5">
      <div class="border border-font h-6 m-1 px-2 text-font text-sm rounded">
        {SPECIALTY_OPTIONS.find(specialty => $performanceFilterStore.specialty[specialty.id] === true).name}
      </div>
      <div class="border border-font h-6 m-1 px-2 text-font text-sm rounded">
        {PERIOD_TYPES_OPTIONS.find(periodType => $performanceFilterStore.periodType === periodType.id).name}
      </div>
      <div class="border border-font h-6 m-1 px-2 text-font text-sm rounded">
        Calendar Year {$performanceFilterStore.period.year}
      </div>
      <div class="border border-font h-6 m-1 px-2 text-font text-sm rounded">
        {$performanceFilterStore.period.type === PERIOD_TIME_TYPES.MONTH
          ? MONTHS_OPTIONS[$performanceFilterStore.period.month]
          : QUARTERS_OPTIONS[$performanceFilterStore.period.quarter]}
      </div>
    </div>
    <Accordion
      ...
      <div class="pt-2" slot="details">
        <List class="ml-2" list="none" data-cy="periodTypeList">
          {#each PERIOD_TYPES_OPTIONS as periodType}
            <Li class="my-2 flex items-center w-full">
              <Radio name="periodType" class="mr-1" color="green" bind:group={filters.periodType} value={periodType.id} />
