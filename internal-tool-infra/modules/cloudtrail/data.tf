data "aws_iam_policy_document" "policy_cloudtrail" {
  statement {
    ...
    resources = ["arn:aws:s3:::${var.tags["Name"]}-cloudtrail-${var.tags["Environment"]}"]
    condition {
      test     = "StringEquals"
  ...
  statement {
    ...
    resources = ["${aws_s3_bucket.s3_cloudtrail.arn}/prefix/AWSLogs/${data.aws_caller_identity.current.account_id}/*"]
